...
$seedFile = Join-Path $projectRoot "migrations\seed_example.sql"
...
if ($parsed) {
    ...
    Write-Output "Running seed with: psql -h $dbHost -p $port -U $username -d $database"
    & psql $psqlArgs
...
if ($exitCode -ne 0) {
  Write-Error "Seed failed with exit code $exitCode"
  exit $exitCode
}
